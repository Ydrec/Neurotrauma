...
local function SpawnSubject(position, name, job, team, dontgivejobitems, dontgiveobjective)
	...
	if not dontgiveobjective then
		...
		local order =
			Order(orderPrefab, OrderTarget(position, hull)).WithManualPriority(CharacterInfo.HighestManualOrderPriority)
...
NTTut.scenarios = {
	...
	dislocations = {
		...
		update = function(item, data)
			if data.character == nil or data.character.Removed then
				return
			end
	...
	fractures = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 4, 1 do
					HF.GiveItem(character, "antibleeding1")
				end
				for i = 1, 4, 1 do
					HF.GiveItem(character, "gypsum")
				end
				data.character = character
			end, 1)
		end,
		...
		update = function(item, data)
			...
			elseif
				(
					not NT.LimbIsBroken(data.character, LimbType.LeftArm)
					or HF.HasAfflictionLimb(data.character, "gypsumcast", LimbType.LeftArm)
				)
				and (not NT.LimbIsBroken(data.character, LimbType.RightArm) or HF.HasAfflictionLimb(
					data.character,
					"gypsumcast",
					LimbType.RightArm
				))
				and (
					not NT.LimbIsBroken(data.character, LimbType.LeftLeg)
					or HF.HasAfflictionLimb(data.character, "gypsumcast", LimbType.LeftLeg)
				)
			then
				state = 2
			end
	...
	skullfracture = {
		...
		update = function(item, data)
			...
			elseif data.character.IsDead then
				state = 0
			elseif not HF.HasAffliction(data.character, "h_fracture") then
				state = 2
			end
	...
	bonedeath = {
		...
		update = function(item, data)
			...
			elseif data.character.IsDead then
				state = 0
			elseif not HF.HasAffliction(data.character, "bonedamage", 90) then
				state = 2
			end
	...
	burns = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 8, 1 do
					HF.GiveItem(character, "antibleeding2")
				end
				for i = 1, 8, 1 do
		...
		update = function(item, data)
			...
			elseif data.character.IsDead then
				state = 0
			elseif burnsPresent < 20 and not HF.HasAffliction(data.character, "sepsis") then
				state = 2
			end
	...
	alkalosis = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 2, 1 do
					HF.GiveItem(character, "antibloodloss1")
				end
		...
		update = function(item, data)
			...
			elseif data.character.IsDead then
				state = 0
			elseif not HF.HasAffliction(data.character, "alkalosis", 5) then
				state = 2
			end
	...
	acidosis = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 4, 1 do
					HF.GiveItem(character, "raptorbaneextract")
				end
				HF.GiveItem(character, "bloodanalyzer")
				data.character = character
			end, 1)
	...
	hypertension = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 2, 1 do
					HF.GiveItem(character, "pressuremeds")
				end
				for i = 1, 2, 1 do
					HF.GiveItem(character, "nitroglycerin")
				end
				HF.GiveItem(character, "bloodanalyzer")
				data.character = character
			end, 1)
	...
	pneumothorax = {
		...
		update = function(item, data)
			...
			elseif data.character.IsDead then
				state = 0
			elseif not HF.HasAffliction(data.character, "pneumothorax") then
				state = 2
			end
	...
	organfailure = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 3, 1 do
					HF.GiveItem(character, "immunosuppressant")
				end
	...
	radiation = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				for i = 1, 2, 1 do
					HF.GiveItem(character, "thiamine")
				end
				data.character = character
			end, 1)
		end,
		...
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				Timer.Wait(function()
					for i = 1, 2, 1 do
						local spawnPos =
							Vector2(item.WorldPosition.X + (((i % 2) - 0.5) * 2 * 400), item.WorldPosition.Y)

						local bandit =
							SpawnSubject(spawnPos, "Bandit", "securityofficer", CharacterTeamType.Team2, true, true)
						...
						HF.SpawnItemPlusFunction("smg", function(params)
							for j = 1, 1, 1 do
								HF.SpawnItemPlusFunction("smgmagazine", nil, nil, params.item.OwnInventory, 0)
							end
						...
						for j = 1, 2, 1 do
							HF.GiveItem(bandit, "smgmagazine")
						end
						for j = 1, 6, 1 do
							HF.GiveItem(bandit, "shotgunshell")
						end
	...
	asphyxiation = {
		startfunc = function(item, data)
			Timer.Wait(function()
				...
				HF.SetAffliction(character, "hypoxemia", 100)
				for i = 1, 2, 1 do
					HF.GiveItem(character, "liquidoxygenite")
				end
				for i = 1, 2, 1 do
					HF.GiveItem(character, "adrenaline")
				end
				for i = 1, 1, 1 do
					HF.GiveItem(character, "mannitol")
				end
...
NTTut.addSignalToCache = function(signal, connection)
	...
	if NTTut.signalCache[connection.Item] == nil then
		NTTut.signalCache[connection.Item] = {}
	end
...
Hook.Add("nttut_subjectcomponent", "nttut_subjectcomponent", function(effect, deltaTime, item, targets, worldPosition)
	if item == nil or item.Submarine == nil or item.Submarine.SubBody == nil then
		return 0
	end
